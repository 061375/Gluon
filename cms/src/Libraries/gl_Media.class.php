...
class gl_Media {
    ...
    {
        $this->config['media'] = \Libraries\gl_Cache::get_cache_byfile('app.yml.php');
        ...
        if($_FILES){
            ...
            for($i = 0; $i < $num ; $i++){
                    ...
                    if($files[$i]['size'] > $this->config['file_size_limit'])
                        $files[$i]['error'] = \Libraries\gl_General::message('error',array('error','upload','sizemax'));
                    if(!in_array($files[$i]['type'],$this->config['allowed_mimetypes']))
                        $files[$i]['error'] = \Libraries\gl_General::message('error',array('error','upload','notallowed'));
            }
    ...
    private function saveFile($file,$path)
    {
        ...
        if(false === $move) {
            $this->errors->set_error_message(\Libraries\gl_General::message('error',array('error','upload','movefile')));
            return false;
        }
        return \Libraries\gl_General::message('notice',array('notice','success','upload'));
    }
    /**
    ...
    private function checkFile($file_path) {
        if(false == file_exists($file_path)) {
            $this->errors->set_error_message(\Libraries\gl_General::message('notice',array('error','upload','upload')));
            return false;
        }
        ...
        if(strpos($test,'<?php') !== false) {
            $this->errors->set_error_message(\Libraries\gl_General::message('notice',array('error','upload','malicious')));
            return false;
        }
        return true;
    }
